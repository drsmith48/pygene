...
class Geqdsk(object):
    ...
    Args:
        geqdsk_file (str, default None): filename for geqdsk file;
    ...
    def __init__(self,
                 geqdsk_file: Union[Path, str] = 'file.geqdsk',
                 plot=False,
                 quiet=False):
        """See class docstring."""
        self.pprime_fs = None
        self.ffprime_fs = None
        self.psirz = None
        self.qpsi_fs = None
        self.p_fs = None
        self.F_fs = None
        ...
        self.plot = plot
        self.quiet = quiet
        self.geqdsk_file = Path(geqdsk_file)
        assert self.geqdsk_file.exists()
        ...
        self.process_geqdsk_file()
        if self.plot:
            self.plot_geqdsk_file()
    ...
    # def load_gfile(self):
    #     self.geqdsk_file = get_filename()
    #     self.process_geqdsk_file()
    #     self.plot_geqdsk_file()

    def process_geqdsk_file(self):

        # read geqdsk file
        with self.geqdsk_file.open('r') as gfile:
        ...
        self.psi_spl = scipy.interpolate.RectBivariateSpline(self.zgrid, self.rgrid, self.psirz,
                                                             kx=self.io,
                                                             ky=self.io)
        ...
        for j, theta in enumerate(self.theta_grid):
            ...
            psi_int = scipy.interpolate.interp1d(psi_rad[:end_ind + 1],
                                                 np.arange(end_ind + 1),
                                                 kind=self.io)
            # near psi-grid index for separatrix
            indsep = int(psi_int(self.psisep)) + 3
            # RZ interp. objects along poloidal line from axis
            R_int = scipy.interpolate.interp1d(psi_rad[:indsep],
                                               r_pol[:indsep],
                                               kind=self.io)
            Z_int = scipy.interpolate.interp1d(psi_rad[:indsep],
                                               z_pol[:indsep],
                                               kind=self.io)
        ...
        for i in range(1, self.nw):
            ...
            Z_int = scipy.interpolate.interp1d(Z_array, R_array, kind=self.io)
    ...
    def miller(
            self,
            psinorm: Union[float, None] = None,
            rova: Union[float, None] = None,
            omt_factor: float = 0.2,
    ):
        """
        ...
        # r and psi splines
        psi_grid_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.psi_grid, k=self.io, s=self.s)
        r_min_spl = scipy.interpolate.UnivariateSpline(self.psi_grid, self.r_minor_fs, k=self.io, s=self.s)
        ...
        output['geqdsk_file'] = self.geqdsk_file
        output['psinorm'] = self.psinorm
        output['rova'] = self.rova

        R0_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.R_major_fs, k=self.io, s=self.s)
        R0_poi = R0_spl(r_poi)[()]  # R_maj of FS
        q_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.qpsi_fs, k=self.io, s=self.s)
        # q_spl_psi = scipy.interpolate.UnivariateSpline(self.psi_grid,   self.qpsi_fs,    k=self.io, s=self.s)
        q_poi = q_spl(r_poi)[()]
        ...
        F_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.F_fs, k=self.io, s=self.s)
        F_poi = F_spl(r_poi)[()]  # F of FS
        Bref_poi = F_poi / R0_poi
        p_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.p_fs, k=self.io, s=self.s)
        p_poi = p_spl(r_poi)[()]
        t_poi = self.ti
        ...
        pprime_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs, self.pprime_fs, k=self.io, s=1e-4)
        ...
        for i, isg in enumerate(subgrid):
            ...
            theta_int = scipy.interpolate.interp1d(theta_mod_ext, theta_tmp,
                                                   kind=self.io)
            R_int = scipy.interpolate.interp1d(theta_mod_ext, R_extended,
                                               kind=self.io)
            Z_int = scipy.interpolate.interp1d(theta_mod_ext, Z_extended,
                                               kind=self.io)
            ...
            for o in range(2):
                ...
                x = R_sym[section]
                y = Z_sym[section]
                y_int = scipy.interpolate.interp1d(x, y, kind=self.io)
            ...
            for o in range(4):
                ...
                if o in [0, 1]:
                    theta_int = scipy.interpolate.interp1d(-searcharr[section], theta_sec, kind=self.io)
                    theta_of_interest = theta_int(-searchval)
                else:
                    theta_int = scipy.interpolate.interp1d(searcharr[section], theta_sec, kind=self.io)
                    theta_of_interest = theta_int(searchval)
                Z_sec = Z_sym[section]
                Z_sec_int = scipy.interpolate.interp1d(theta_sec, Z_sec, kind=self.io)
        ...
        amhd_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs[sgstart:], amhd, k=self.io, s=1e-3)
        drR_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs[sgstart:], drR, k=self.io, s=self.s)
        b_spl = scipy.interpolate.UnivariateSpline(self.psinorm_grid[sgstart:], b, k=self.io, s=self.s)

        # calc derivatives for kappa, delta, zeta
        s_kappa = np.empty(nsg)
        s_delta = np.empty(nsg)
        s_zeta = np.empty(nsg)
        kappa_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs[sgstart:], kappa, k=self.io, s=self.s)
        delta_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs[sgstart:], delta, k=self.io, s=self.s)
        zeta_spl = scipy.interpolate.UnivariateSpline(self.r_minor_fs[sgstart:], zeta, k=self.io, s=self.s)
        for i, isg in enumerate(subgrid):
...
if __name__ == '__main__':
    geqdsk_file = REPO_DIR / 'geqdsk/pegasus-eq21.geqdsk'
    assert geqdsk_file.exists()
    geq = Geqdsk(geqdsk_file=geqdsk_file, plot=True)
    miller = geq.miller()
    if not plt.isinteractive():
        plt.show()
