...
#SBATCH -N 8 --ntasks-per-node=32 --cpus-per-task=2 --gpus-per-node=0 -C rh8
#SBATCH --mail-user=drsmith@pppl.gov --mail-type=END,FAIL
#SBATCH -o run-%j.out
###SBATCH --exclude=traverse-k01g[1-12]
...
ls -lt
echo "running GENE"
srun ${GENEBIN}/gene_arch_pgi_204_debug0_cuda0.none > gene.out
exitcode=$?
sleep 5
