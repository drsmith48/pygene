...
def miller(r_a=0.76, gfile='pegasus-eq21.geqdsk'):
    ...
    for j in range(ntheta):
        ...
        R_ft[:,j] = R_int(psi_uni_grid) # R_ft coords of FS grid at fixed theta
        Z_ft[:,j] = Z_int(psi_uni_grid) # Z coords of FS grid at fixed theta

    #find average elevation for all flux surfaces
    Z_avg_fs = np.empty(nw,dtype=float)
    for i in range(nw):
        ds = np.empty(ntheta,dtype=float)
        ds[1:ntheta-1]=0.5*np.sqrt((R_ft[i,2:ntheta]-R_ft[i,0:ntheta-2])**2+(Z_ft[i,2:ntheta]-Z_ft[i,0:ntheta-2])**2)
    ...
    psi_uni_grid_spl=US(r_min_fs,psi_uni_grid,k=interpol_order,s=1e-5)
    ...
    r=r_a*r_min_fs[-1]  # r/a * a = r; FS minor radius
    ...
    r_min_spl=US(psi_uni_grid,r_min_fs,k=interpol_order,s=1e-5)
    ...
    poi_ind=find(r_a,r_min_fs//r_min_fs[-1])
    ...
    for i in psi_stencil:
        ...
        dV_dr[imod]=abs(sum(qpsi[i]*R_sym[imod]**2/F[i])*4*np.pi**2/ntheta)/r_min_spl.derivatives(psi_uni_grid[i])[1]
    #    V[imod]=trapz(Vprime[:imod+1],psi_uni_grid[psi_stencil])
        ...
        amhd[imod]=-qpsi[i]**2*R_maj_fs[i]*pprime[i]*8*np.pi*1e-7/Bref_miller**2/r_min_spl.derivatives(psi_uni_grid[i])[1]
    #    amhd_Miller[imod]=-2*Vprime[imod]/(2*np.pi)**2*(V[imod]/2/np.pi**2/R_maj_fs[i])**0.5*4e-7*np.pi*pprime[i]
        dq_dr_avg[imod]=q_spl.derivatives(r_min_fs[i])[1]
...
#    plt.axvline(r,0,1,ls='--',color='k',lw=2)
    ...
    ind=poi_ind-psi_stencil[0]#find(r_a,r_min_fs/r_min_fs[-1])
    #Lref=R0_pos
